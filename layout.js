/** PAGE: layout.js | VERSION: 1.9.6 */
/** * DESCRIPTION: Gestion de l'UI globale et du Popup Modal PWA intelligent.
 * Détecte iOS pour afficher un guide manuel et Android pour l'installation directe.
 */
...
function injectPWAPopup() {
    ...
    <div id="pwa-modal" style="display:none; position:fixed; inset:0; z-index:10000; background:rgba(29,67,121,0.7); backdrop-filter:blur(10px); align-items:center; justify-content:center; padding:20px;">
        <div style="background:white; width:100%; max-width:340px; border-radius:35px; padding:30px; text-align:center; box-shadow:0 25px 50px rgba(0,0,0,0.3); border:1px solid rgba(255,255,255,0.3);">
    ...
    // Affichage auto après 3 secondes sur mobile uniquement
    if (window.innerWidth < 1024) {
        window.addEventListener('beforeinstallprompt', (e) => {
            e.preventDefault(); window.deferredPrompt = e;
            setTimeout(() => { document.getElementById('pwa-modal').style.display = 'flex'; if(window.lucide) lucide.createIcons(); }, 3000);
        });
        if (isIOS) setTimeout(() => { document.getElementById('pwa-modal').style.display = 'flex'; if(window.lucide) lucide.createIcons(); }, 3000);
    }
...
function injectMobileMenu() {
    ...
    <div id="mobile-menu" style="position:fixed; top:0; left:0; bottom:0; width:280px; background:white; z-index:2000; padding:24px; display:flex; flex-direction:column; transform:translateX(-100%); transition:0.3s ease;">
        ...
        <nav style="display:flex; flex-direction:column; gap:15px; font-weight:800; color:#1d4379; text-transform:uppercase; font-size:11px;">
            <a href="index.html" style="text-decoration:none; color:inherit; display:flex; gap:12px;"><i data-lucide="home"></i> ${DjuntaT('nav_home')}</a>
            <a href="search-driver.html" style="text-decoration:none; color:inherit; display:flex; gap:12px;"><i data-lucide="steering-wheel"></i> ${DjuntaT('nav_driver')}</a>
            <a href="search-car.html" style="text-decoration:none; color:inherit; display:flex; gap:12px;"><i data-lucide="car-front"></i> ${DjuntaT('nav_car')}</a>
            <a href="my-rentals.html" style="text-decoration:none; color:inherit; display:flex; gap:12px;"><i data-lucide="calendar"></i> ${DjuntaT('nav_rentals')}</a>
            <a href="wallet.html" style="text-decoration:none; color:inherit; display:flex; gap:12px;"><i data-lucide="wallet"></i> ${DjuntaT('nav_wallet')}</a>
            <a href="profile.html" style="text-decoration:none; color:inherit; display:flex; gap:12px;"><i data-lucide="user-circle"></i> ${DjuntaT('nav_account')}</a>
        </nav>
        <button onclick="handleLogout()" style="margin-top:auto; background:#fef2f2; color:#ef4444; border:none; padding:15px; border-radius:12px; font-weight:900; font-size:12px; display:flex; align-items:center; justify-content:center; gap:8px; cursor:pointer;">
...
function injectFooter() {
    ...
    <footer id="master-footer" style="padding:40px 20px 200px; text-align:center; background:#f8fafc; border-top:1px solid #f1f5f9;">
        ...
        <p style="font-size:9px; color:#cbd5e1; font-weight:700;">© 2026 DJUNTACAR • v1.9.6</p>
    </footer>`);
}
